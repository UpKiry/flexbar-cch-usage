# Flexbar CC Hub 用量插件计划

## 产品目标

在 Flexbar 上提供一个稳定、低频刷新的 CC Hub 用量仪表盘。用户完成一次配置后，可以在设备上快速查看配额和可选时间范围的用量，不需要打开网页或命令行。

## V1：可靠仪表盘

### 功能范围

- 插件配置页：CC Hub URL、API Key、刷新间隔；
- 两个固定按键：配额、用量；
- 启动、设备连接、按键点击和定时自动刷新；
- 保留上一次成功数据，并在数据过期时给出状态提示；
- 使用 CC Hub 登录会话获取用量数据；
- 配置和 API Key 由 FlexDesigner 管理，不写入按键内容或日志。

### V1 交付任务

- [x] 完成配置页和配置持久化；
- [x] 完成两个固定按键的基础显示；
- [x] 接入配额、今日用量和日期汇总接口；
- [x] 完成启动、点击、设备连接和定时刷新；
- [x] 完成 Cookie 复用、重新登录和有限重试；
- [x] 完成错误状态和旧数据保留；
- [x] 使用内嵌 CJK 字体和按键级 PNG 自绘，固定布局为 240×60；
- [x] 按设备和按键视觉指纹去重，数据不变时跳过图片发送；
- [x] 覆盖 180px 紧凑布局、极端数据和字体字符集测试；
- [x] 补充 CC Hub 客户端 mock 测试；
- [x] 补充刷新并发、缓存和配置更新测试；
- [x] 补充 FlexDesigner 事件和 `plugin.draw()` mock 测试；
- [x] 完成设备拔插、多设备和长时间运行验收。
- [x] 在 macOS 实际 Flexbar 上确认自绘 PNG 的安装、刷新和最终字形效果。

## V1 数据接口

插件使用以下 CC Hub 接口：

- `POST /api/auth/login`
- `GET /api/v1/me/quota`
- `GET /api/v1/me/today`
- `GET /api/v1/me/usage-logs/stats-summary`

第一版不在常驻按键中展示完整调用日志。

## V1 验收标准

- 配置页可以保存并重新打开后保持原值；
- 正确的 CC Hub URL 和 API Key 可以显示实际用量；
- 两个按键标题在真实 Flexbar 上保持可读；
- API 失败时不清空旧数据，也不泄露 API Key 或 Cookie；
- 插件重启、设备重连和配置更新后能够恢复刷新；
- 同时连接多个设备时，各设备按键状态互不覆盖；
- 插件可在 macOS 和 Windows FlexDesigner 上安装运行。

### V1 验收记录（2026-09-12）

- Node.js mock 已覆盖 CC Hub 登录/接口、有限重试、刷新并发、配置更新、缓存保留、多设备隔离、事件分发和 `plugin.draw()` 去重。
- macOS 本机已完成 `npm run build`、`npm run plugin:validate`、`npm run plugin:pack`，并通过 FlexDesigner `plugin:install --force` 安装 `1.2.0`；插件进程成功连接本地 FlexDesigner WebSocket。
- 已检测到真实 Flexbar（ENIAC，序列号 `D41000BA2010`），并修复 SDK 断线重连时丢失端口导致的 backend 崩溃；重启后连接稳定观察超过 60 秒。发布包已补入 Windows x64 canvas 原生模块，并通过包清单检查。
- 用户已确认真实环境 V1 验收通过：配置保存与重开、实际用量显示、四键 PNG 字形与刷新、设备重连/拔插、多设备隔离、长时间运行，以及 macOS/Windows FlexDesigner 安装运行均已验证；测试与日志均未写入 API Key 或 Cookie。

## 1.3.0：双按键与按键级时间范围

- 按键库仅保留配额和用量；用量按键使用独立 `usage` CID。
- 用量按键支持 `5h`、`1d`、`7d`、`1m`，默认 `1d`，每个按键独立保存范围。
- `5h` 使用 quota 当前消费金额；`1d` 使用今日接口；`7d` 和 `1m` 使用最近 7 天、30 天汇总。
- 配额按键固定 5 小时周期，显示当前使用金额 / 配额上限；配置页仅展示固定周期说明。
- 配置页时间范围显示为“5 小时”“1 天”“7 天”“30 天”。测试连接不保存配置，保存成功后重启插件并加载最新数据。
- 保存配置后在用户目录保留权限受限的恢复副本，插件重新安装后自动恢复宿主配置。
- 旧 `today` 按键不自动迁移；现有 quota CID 保持不变。

## V2：详情信息

在 V1 稳定后增加：

- 最近调用记录；
- 模型分布；
- 点击按键查看详细信息；
- 更细粒度的用量明细。

## V3：视觉与账户扩展

在 V2 稳定后评估：

- 动态按键；
- 配额进度和阈值视觉提示；
- 刷新倒计时和连接状态；
- 多账户或多 CC Hub 配置。

## 完成定义

V1 的实现、自动化测试和真实设备验收全部通过后，才标记为完成并进入 V2。版本号只在可安装包完成验证后推进。
